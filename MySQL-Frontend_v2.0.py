    ...
    def goFwd(self):
        ...
        if col_TBR[0] == None and self.renameCol_textBox.text() == '':
            ...
            if col_TBR[0] == None:
                popup=QMessageBox.about(self,"Error","No Column is Selected.")
                print("\aNo Column is Selected.")
            elif self.renameCol_textBox.text() == '':
...
class update_table_confirmation_querybox(QDialog):
    ...
    def textPopup(self,item): # popup window allowing user to edit queries for exec method execute2()
...
All update queries have been executed.\nIncase you want to revert back from the update, you can get insert_data queries saved as a .txt file in the current \
    ...
    def exeCUTE2_FINAL(self):
        ...
        try:
            ...
            for QUERY in MASTER_QUERY_LIST:
                ...
                i += 1
            myconn.commit()
            print("UPDATE_DATA() EXECUTE2(): ALL QUER(y/ies)() EXECUTED.\nEXECUTED {} QUERIES.".format(i))
...
class wel_Login_scr(QDialog):
    ...
    def helpPopup(self):
        ...
        msg="""
This program is designed to work on MySQL Server v5.1.33 installed
...
class MainWindow(QDialog):
    def __init__(self):
        ...
        self.TBlistwidget.itemDoubleClicked.connect(self.selectTB)
    ...
    def queryGEN1(self,item):    # generates query for table(s) WITH PRIMARY KEY
        ...
        if item == self.tableWidget.currentItem():
            ...
            if update_DATA.primary_key_exists:
                ...
                if corr_PRI_DATA == 'None' or corr_PRI_DATA == 'NULL':  # fixed None <--> NULL bug
                    corr_PRI_DATA = 'NULL'                                   #fetches old-UNEDITED primary_key data
                ...
                try:
                    ...
                    if corr_PRI_DATA.isalnum(): #saves query with quotes for the corresponding primary key data
                        Query +=" {}=\"{}\";".format(PRI_Key,corr_PRI_DATA)
                    elif corr_PRI_DATA == 'NULL':
                        Query +=" {}={};".format(DB_TB_choice[1], PRI_Key , corr_PRI_DATA)
                    else:
                        Query +=" {}=\"{}\";".format(PRI_Key,corr_PRI_DATA)
            ...
            for i in range(1,row_count): #for every row iteration
                ...
                for k in range(1,len(list(DATA[i]))): #for every col ite
                    try:
                        ...
                        if DATA[i][k] == None or DATA[i][k].isdigit():
                            DATA[i][k] = 'NULL' #fixed None <--> NULL bug
                            row += "{}, ".format(DATA[i][k])
                        elif DATA[i][k].isalnum(): # saves it as String
                            row += "\"{}\", ".format(DATA[i][k])
